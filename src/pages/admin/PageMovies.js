  ...
  async main() {
    ...
    for (const item of data) {
      HTML += `
                    <tr>
                        <td>${item.id}</td>
                        <td><img style="max-width: 5rem; max-height: 5rem;" src="/img/movie-thumbnails/${
                          item.thumbnail
                        }" alt="Movie thumbnail"></td>
                        <td>${item.title}</td>
                        <td>${item.url_slug}</td>
                        <td>${
                          item.description
                            ? '<div class="badge rounded-pill bg-success">Provided</div>'
                            : '<div class="badge rounded-pill bg-danger">Missing</div>'
                        }</td>
                        <td>${item.duration}</td>
                        <td>
